...
<div class="card">
    <div class="card-body">
        <form action="{{route('admin.term-condition.update',['id'=>$term_condition->id])}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <div class="row pb-3">
                <div class="form-group col-12">
                    <label>Title</label>
                    <input type="text" class="form-control" name="title" required value="{{$term_condition->title}}">
                    @if($errors->has('title'))
